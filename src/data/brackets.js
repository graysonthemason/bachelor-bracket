...
brackets.forEach(({name, picks})=>{
  var results = [];
  var arr = picks;
  if (arr.length !== 18) console.log(`not the right num of entries for ${name} (${arr.length})`)
  ...
  if (results.length > 0 ) console.log(`Duplicates in Bracket for ${name} (id: ${results[0]})`)
})
